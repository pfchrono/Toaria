    ...
    public class Collision
    {
        ...
        public static Vector2 HurtTiles(Vector2 Position, Vector2 Velocity, int Width, int Height, bool fireImmune = false)
        {
            ...
            for (int i = num; i < maxTilesX; i++)
            {
                ...
                {
                    Vector2 vector;
                    if (((Main.tile[i, j] == null) || !Main.tile[i, j].active) || ((((Main.tile[i, j].type != 0x20) && (Main.tile[i, j].type != 0x25)) && ((Main.tile[i, j].type != 0x30) && (Main.tile[i, j].type != 0x35))) && (((Main.tile[i, j].type != 0x39) && (Main.tile[i, j].type != 0x3a)) && (((Main.tile[i, j].type != 0x45) && (Main.tile[i, j].type != 0x4c)) && (Main.tile[i, j].type != 80)))))
                    {
                        continue;
                    }
                    vector.X = i * 0x10;
