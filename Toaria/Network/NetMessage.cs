                ...
                if (!NetHooks.OnSendData(ref msgType, ref remoteClient, ref ignoreClient, ref text, ref number, ref number2, ref number3, ref number4))
                {
                        ...
                        for (int num88 = 0; num88 < 0x100; num88++)
                        {
                            if (((num88 != ignoreClient) && (buffer[num88].broadcast || ((Netplay.serverSock[num88].state >= 3) && (msgType == 10)))) && Netplay.serverSock[num88].tcpClient.Connected)
                            {
                                try
                                {
                                    ...
                                    buffer226.spamCount++;
                                    SendBytes(Netplay.serverSock[num88], buffer[index].writeBuffer, 0, count, new AsyncCallback(Netplay.serverSock[num88].ServerWriteCallBack), Netplay.serverSock[num88].networkStream);
                                }
                                catch
                                {
                                }
                    ...
                    else if (Netplay.serverSock[remoteClient].tcpClient.Connected)
                    {
                        try
                        {
                            ...
                            buffer227.spamCount++;
                            SendBytes(Netplay.serverSock[remoteClient], buffer[index].writeBuffer, 0, count, new AsyncCallback(Netplay.serverSock[remoteClient].ServerWriteCallBack), Netplay.serverSock[remoteClient].networkStream);
                        }
                        catch
                        {
                        }
